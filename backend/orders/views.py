...
def get_customer_order(request, order_number, lock=False):
...
def restore_cancelled_order_resources(order):
    """Restore stock and coupon usage exactly once after cancellation."""
    ...
    for item in order_items:
        ...
        variant.stock = stock_before + quantity
    ...
    if coupon_usage is not None:
        coupon_id = coupon_usage.coupon_id
        coupon_usage.delete()

        coupon_model = CouponUsage._meta.get_field(
            "coupon"
        ).remote_field.model
...
class CheckoutView(generics.CreateAPIView):
    ...
    Guest aur logged-in, dono customers order create kar sakte hain.
    """
...
class RazorpayCreateOrderView(APIView):
    ...
    Expected body:
    {
        "order_number": "YUV-XXXXXXXXXX",
        "phone": "9876543210"
    }

    Logged-in customer ke liye phone optional hai.
    Guest customer ke liye phone required hai.
    """
    ...
    def post(self, request):
        ...
        try:
            ...
            payment.failure_description = str(error)
        ...
        payment.gateway_order_id = razorpay_order.get(
            "id"
        )
...
class RazorpayVerifyPaymentView(APIView):
    ...
    def post(self, request):
        ...
        gateway_amount = razorpay_payment.get(
            "amount"
        )
        ...
        gateway_status = razorpay_payment.get(
            "status",
            "",
        )
...
class InvoiceDownloadView(APIView):
    ...
    Logged-in customer apne order ka PDF invoice download kar sakta hai.
    """
    ...
    def get(self, request, order_number):
        ...
        story = [
            Paragraph("YUVON", styles["InvoiceTitle"]),
            Paragraph("Tax Invoice", styles["Heading2"]),
            Spacer(1, 5 * mm),
        ]

        placed_at = getattr(order, "placed_at", None) or getattr(
            order, "created_at", None
        )
        order_date = (
            timezone.localtime(placed_at).strftime("%d %b %Y, %I:%M %p")
            if placed_at
            else "-"
        )
        ...
        customer_email = getattr(order, "email", "") or getattr(
            request.user, "email", ""
        )
        ...
        billing_text = "<br/>".join(
            filter(
                None,
                [
                    f"<b>{customer_name}</b>" if customer_name else "",
                    customer_email,
                    customer_phone,
                    *address_parts,
                ],
            )
        ) or "Customer details unavailable"

        payment_method = str(
            getattr(order, "payment_method", "COD") or "COD"
        ).upper()
        payment_status = str(
            getattr(order, "payment_status", "pending") or "pending"
        ).title()
                ...
                [
                    ...
                    Paragraph(
                        (
                            ...
                            f"<b>Order Status:</b> {str(order.status).title()}<br/>"
                            f"<b>Payment:</b> {payment_method} / {payment_status}"
                        ),
                        styles["InvoiceRight"],
                    ),
                ]
        ...
        item_rows = [["#", "Product", "Variant", "Qty", "Price", "Amount"]]
        ...
        for index, item in enumerate(order.items.all(), start=1):
            ...
            variant_parts = []
            for source in (item, variant):
                if not source:
                    continue
                for field_name, label in (("size", "Size"), ("color", "Color")):
                    value = getattr(source, field_name, "")
                    if value and f"{label}: {value}" not in variant_parts:
                        variant_parts.append(f"{label}: {value}")
            ...
            line_total_value = getattr(item, "total_price", None) or getattr(
                item, "subtotal", None
            )
            line_total = (
                Decimal(str(line_total_value))
            ...
            item_rows.append(
                [
                    str(index),
                    Paragraph(str(product_name), styles["InvoiceSmall"]),
                    Paragraph(", ".join(variant_parts) or "-", styles["InvoiceSmall"]),
                    str(int(quantity)),
                    f"Rs. {unit_price:.2f}",
                    f"Rs. {line_total:.2f}",
                ]
        ...
        items_table = Table(
            item_rows,
            colWidths=[9 * mm, 60 * mm, 38 * mm, 14 * mm, 24 * mm, 27 * mm],
            repeatRows=1,
        )
        ...
        subtotal = Decimal(
            str(getattr(order, "subtotal", calculated_subtotal) or calculated_subtotal)
        )
        discount = Decimal(str(getattr(order, "discount_amount", 0) or 0))
        shipping = Decimal(str(getattr(order, "shipping_charge", 0) or 0))
        ...
        if shipping:
            totals_data.append(["Shipping", f"Rs. {shipping:.2f}"])
        if tax:
            totals_data.append(["Tax", f"Rs. {tax:.2f}"])
        totals_data.append(["Grand Total", f"Rs. {total:.2f}"])

        totals_table = Table(
            totals_data,
            colWidths=[42 * mm, 35 * mm],
            hAlign="RIGHT",
        )
...
class AdminOrderListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        state_name = self.request.query_params.get("state", "").strip()
        ...
        if state_name:
            queryset = queryset.filter(state__iexact=state_name)
...
class AdminOrderUpdateView(generics.UpdateAPIView):
    ...
    Updates status, payment status, admin note and courier details.
    """
    ...
    def update(self, request, *args, **kwargs):
        ...
        response_serializer = AdminOrderDetailSerializer(
            order,
...
class AdminDashboardSummaryView(APIView):
    ...
    def get(self, request):
        ...
        payment_status_counts = {
            ...
            for row in queryset.values("payment_status").annotate(
                count=Count("id")
            )
        }
        ...
        return Response(
            {
                ...
                "today": {
                    "orders": today_summary["orders"] or 0,
                    "paid_revenue": (
                        today_summary["revenue"]
                        or Decimal("0.00")
                    ),
                },
            },
...
# Backward-compatible alias for older imports/routes.
AdminOrderDashboardView = AdminDashboardSummaryView
