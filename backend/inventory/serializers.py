...
class InventorySerializer(serializers.ModelSerializer):
    ...
    def _get_threshold(self):
        threshold = self.context.get("low_stock_threshold")
    ...
    def get_available_stock(self, obj):
        return max(0, int(obj.stock or 0))
    ...
    def get_is_low_stock(self, obj):
        ...
        return 0 < stock <= self._get_threshold()
...
class StockAdjustmentSerializer(serializers.Serializer):
    ...
    def validate_variant_id(self, value):
        ...
        self.context["variant_object"] = variant
        return value

    def validate(self, attrs):
        variant = self.context.get("variant_object")
        adjustment_type = attrs["adjustment_type"]
        quantity = attrs["quantity"]

        if adjustment_type in {"add", "remove"} and quantity < 1:
            raise serializers.ValidationError(
                {
                    "quantity": (
                        "Quantity must be at least 1 for add or remove."
                    )
                }
            )

        current_stock = max(0, int(getattr(variant, "stock", 0) or 0))
    ...
    def get_stock_change(self):
        if not hasattr(self, "validated_data"):
            raise AssertionError(
                "Call is_valid() before get_stock_change()."
            )

        variant = self.context["variant_object"]
...
class InventorySummarySerializer(serializers.Serializer):
    ...
    active_variants = serializers.IntegerField(read_only=True)
    total_stock = serializers.IntegerField(read_only=True)
    low_stock_variants = serializers.IntegerField(read_only=True)
    out_of_stock_variants = serializers.IntegerField(read_only=True)
